            ...
            if (!fileIsSymbolicLink) {
                ...
                if (fp) {
                    if ([[[fullPath pathExtension] lowercaseString] isEqualToString:@"zip"]) {
                        DebugLog(@"Unzipping nested .zip file:  %@", [fullPath lastPathComponent]);
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    BOOL success = NO;
    ZipUnzip *zipArchive = [[ZipUnzip alloc] initWithPath:path];
